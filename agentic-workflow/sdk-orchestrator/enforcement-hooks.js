...
// ─── Enforcement Utilities (extracted) ──────────────────────────────────────
const {
...
function createEnforcementHooks(agentName, options = {}) {
    ...
    hooks.onPreToolUse = async (input, invocation) => {
        ...
        // Glass verb detection (server key 'glass' → tool names like mcp__glass__open).
        // Match the trailing verb segment robustly across naming styles.
        const glassVerb = String(toolName).split(/[^a-zA-Z]+/).filter(Boolean).pop() || '';
        const isGlassNav = glassVerb === 'open';
        const isGlassSee = glassVerb === 'see';
        ...
        if (agentName === 'scriptgenerator') {
            ...
            if (explorationEnabled && state.toolCallCount === 1) {
                const isFirstMcpNavigationAction = toolName.includes('unified_navigate') ||
                    toolName.includes('unified_crawl') || isGlassNav ||
                ...
                if (!isFirstMcpNavigationAction) {
                    ...
                    return {
                        ...
                        additionalContext:
                            ...
                            'Allowed first calls:\n' +
                            '1. open (Glass — navigate to the target URL) ← preferred\n' +
                            '2. unified_navigate\n' +
                            '3. unified_crawl (autonomous DevTools crawl from a startUrl)\n' +
                            '4. unified_execute_exploration with a script/template that includes navigate\n\n' +
                            `Received: ${toolName}`,
                    };
                }
            }
        ...
        if (agentName === 'scriptgenerator') {
            ...
            // ── Glass verbs credit the same exploration gates as their unified peers ──
            if (isGlassNav) {
                state.mcpNavigateCalled = true;
                registerVisitedPage(state, toolArgs.url || toolArgs.href, 'pre:glass:open');
                log('✅ Glass open — navigation/exploration started');
            }
            if (isGlassSee) {
                state.mcpSnapshotCalled = true;
                state.mcpSelectorValidated = true;
                markPageEvidence(state, 'selector', 'glass:see');
                log('✅ Glass see — affordances perceived & durable handles captured');
            }
            if (glassVerb === 'do') {
                state.mcpSelectorValidated = true;
                markPageEvidence(state, 'selector', 'glass:do');
            }
            if (glassVerb === 'read') {
                const what = toolArgs.what || 'text';
                if (what === 'url' || what === 'title') {
                    state.mcpUrlVerified = true;
                    markPageEvidence(state, 'url', 'glass:read');
                } else {
                    state.mcpContentExtracted = true;
                    markPageEvidence(state, 'content', 'glass:read');
                }
            }
            if (glassVerb === 'wait') {
                ...
                if (toolArgs.for === 'url') {
                    state.mcpUrlVerified = true;
                    markPageEvidence(state, 'url', 'glass:wait');
                }
            }
            ...
            // ── Intelligent primitives & autonomous crawler satisfy exploration gates ──
            // These high-level tools perform real, self-healing exploration server-side,
            // so they credit the same checks the low-level tools do (no double work).
            if (toolName.includes('unified_observe')) {
                state.mcpSnapshotCalled = true;
                state.mcpSelectorValidated = true;
                markPageEvidence(state, 'selector', toolName);
                log('✅ MCP observe — live selectors ranked & validated');
            }
            if (toolName.includes('unified_extract')) {
                state.mcpContentExtracted = true;
                markPageEvidence(state, 'content', toolName);
                log('✅ MCP extract — assertion data captured');
            }
            if (toolName.includes('unified_act')) {
                state.mcpSnapshotCalled = true;
                state.mcpSelectorValidated = true;
                markPageEvidence(state, 'selector', toolName);
                log('✅ MCP act — element resolved & actioned on the live page');
            }
            if (toolName.includes('unified_crawl')) {
                state.mcpNavigateCalled = true;
                state.mcpSnapshotCalled = true;
                state.mcpSelectorValidated = true;
                state.mcpContentExtracted = true;
                state.mcpUrlVerified = true;
                registerVisitedPage(state, toolArgs.startUrl || toolArgs.url, 'pre:crawl');
                log('✅ MCP crawl — full DevTools exploration satisfied all exploration gates');
            }
    ...
    hooks.onPostToolUse = async (input, invocation) => {
        ...
        if (toolName.includes('unified_snapshot') || glassVerb === 'see') {
