...
async function runTests() {
    ...
    // Test 31: runtime blocker observations are surfaced through onPostToolUse
    {
        const { createEnforcementHooks } = require('./enforcement-hooks');
        const hooks = createEnforcementHooks('scriptgenerator', { config: {}, verbose: false });
        const sessionId = `runtime-blocker-${Date.now()}`;
        const inv = { sessionId };

        const result = await hooks.onPostToolUse({
            toolName: 'unified_check',
        ...
        assert(result.additionalContext.includes('RUNTIME BLOCKER OBSERVED'), 'Runtime blocker warning returned from onPostToolUse');
        assert(result.additionalContext.includes('unified_handle_dialog'), 'Runtime blocker guidance suggests dialog handling');
    }

    // Test 32: runtime blocker errors are classified before generic timeout retry guidance
    {
        const { createEnforcementHooks } = require('./enforcement-hooks');
        const hooks = createEnforcementHooks('scriptgenerator', { config: {}, verbose: false });
        const sessionId = `runtime-blocker-error-${Date.now()}`;
        const inv = { sessionId };
        ...
        const errorResult = await hooks.onErrorOccurred({
            error: 'Tool execution failed [RUNTIME_BLOCKER]: Blocked by modal/overlay while executing unified_click',
            errorContext: 'unified_click',
        }, inv);

        assertEq(errorResult.errorHandling, 'retry', 'Runtime blocker uses retry path');
        assert(errorResult.additionalContext.includes('blocking modal/overlay'), 'Runtime blocker error includes blocker-specific guidance');
    }

    // Test 33: Structured blocker telemetry is reflected in OODA metrics and warnings
    {
        const qa = new ExplorationQualityAnalyzer({ config: {} });
        const result = qa.assess({
            blockerState: {
        ...
        assertEq(result.metrics.blocker.present, true, 'OODA metrics capture blocker presence');
        assertEq(result.metrics.blocker.autoRecoverable, true, 'OODA metrics capture autoRecoverable blockers');
        assert(result.warnings.some(w => w.includes('Recoverable blocker detected')), 'Structured blocker warning raised');
        assert(result.warnings.some(w => w.includes('Target occlusion detected')), 'Target occlusion warning raised');
        assert(result.warnings.some(w => w.includes('Focus trap detected')), 'Focus trap warning raised');
    }

    // Test 34: onPostToolUse keeps runtime blocker context and OODA context together for snapshots
    {
        const { createEnforcementHooks } = require('./enforcement-hooks');
        const hooks = createEnforcementHooks('scriptgenerator', {
            config: { ooda: { explorationQuality: { minElements: 5, minRoleDiversity: 3, retryThreshold: 30, warnThreshold: 60 } } },
        ...
        const sessionId = `runtime-blocker-ooda-${Date.now()}`;
        const inv = { sessionId };
        ...
        assert(postResult.additionalContext.includes('RUNTIME BLOCKER OBSERVED'), 'Runtime blocker context preserved during snapshot assessment');
        assert(postResult.additionalContext.includes('OODA SNAPSHOT QUALITY'), 'OODA quality context is still emitted for blocker snapshots');
    }

    // Test 35: Non-recoverable blockers abort instead of retrying forever
    {
        const { createEnforcementHooks } = require('./enforcement-hooks');
        const hooks = createEnforcementHooks('scriptgenerator', { config: {}, verbose: false });
        const sessionId = `runtime-blocker-auth-${Date.now()}`;
        const inv = { sessionId };
        ...
        assertEq(errorResult.errorHandling, 'abort', 'Non-recoverable blockers abort instead of retrying');
        assert(errorResult.additionalContext.includes('Do NOT blindly retry'), 'Abort guidance explains why retry is unsafe');
    }
