...
After displaying in chat, also generate Excel: `scripts/excel-template-generator.js`
Export to: `test-cases/<TICKET-ID>.xlsx`
...
async function generateTestCaseExcelFromJira(ticketUrl) {
  ...
  const ticketData = await fetchJiraTicket({
    cloudId: '<JIRA_CLOUD_ID from .env or workflow-config.json>',
...
**ENSURE:** When generating test cases using Jira ticket URL, use the values from `workflow-config.json` or `.env`:
```json
{
