...
tools: ['search/fileSearch', 'search/textSearch', 'search/listDirectory', 'web/fetch', 'edit', 'search/changes', 'search/codebase', 'read/readFile','execute/getTerminalOutput', 'execute/runInTerminal','read/terminalLastCommand','read/terminalSelection', 'unified-automation-mcp/*']
user-invokable: true
---
...
> **Dynamic Paths:** Script output directory, import paths, and framework patterns are resolved from `workflow-config.json → projectPaths`. If `frameworkMode` is `"basic"`, generate standalone Playwright tests without launchBrowser/POmanager imports.
...
### PHASE 2: SCRIPT GENERATION (only after Phase 1 is 100% complete)
...
## 🎯 MCP TOOL CATEGORIES (90+ Capabilities)
...
The assertion system is configurable via `assertion-config.json`:
...
| File | Purpose |
|------|---------|
| `assertion-config.json` | Main configuration with all frameworks |
| `assertion-config.schema.json` | JSON Schema for validation |
...
✅ ALWAYS include: `test.describe()` blocks, helper functions, test annotations, browser cleanup, error handling
...
## 🚫 POPUP HANDLING — USE FRAMEWORK PAGE OBJECTS

**CRITICAL: Use existing framework Page Objects for popup handling. DO NOT generate custom popup code when framework handlers exist.**
...
### Framework Popup Handlers

| Popup Type | Page Object | Method/Locator | Usage |
|------------|-------------|----------------|-------|
| Welcome Modal | `agentBranding.js` | `agentBrandingContainer`, `continueCTA` | Check `isEnabled()` then click `continueCTA` |
| Tour/Compare | `compareAllPopUp.js` | `skipAllComparePopUp()` | Call method directly |
| Welcome Container | `welcomePopUp.js` | `welcomeModal`, `closeButton` | Modal detection |
| Authentication | `authenticationPopUp.js` | Via POmanager | `Pomanager.authenticationPopUp()` |
| Off-limits Agent | `offLimitsAgentPopUp.js` | Via POmanager | `Pomanager.offLimitsAgentPopUp()` |

### ✅ REQUIRED Pattern: Agent Branding Popup

```javascript
// USE THIS — from tests/pageobjects/agentBranding.js
...
### ✅ REQUIRED Pattern: Tour/Compare Popup

```javascript
// USE THIS — from tests/pageobjects/compareAllPopUp.js
await skippAllComparePopUp.skipAllComparePopUp();
```

**❌ NEVER generate custom tour popup handlers:**
```javascript
// WRONG
const tourPopup = page.locator('.tour-step');
if (await tourPopup.isVisible()) { await page.locator('[data-qa="skip-all-highlight-popout"]').click(); }
```

### Detection Flow

```
1. During MCP exploration, detect popup type
2. Check against framework mapping:
   '[data-test-id="welcome-modal"]' → USE agentBrandingPage
   '.tour-step' → USE skippAllComparePopUp
3. If match → MUST use framework Page Object
4. If UNKNOWN → Generate minimal custom handler + comment: // TODO: Consider adding to framework
```
...
|----------|------|-------------|
| `GeneralFunctions` | `business-functions/general.js` | `open()`, `openOneHome()`, `waitForMapIsLoaded()` |
| `LoginFunctions` | `business-functions/login.js` | `signIn()`, `enterCredentialsClickSignIn()`, `signInAndWaitForMapIsLoaded()`, `signOut()` |
| `SearchFunctions` | `business-functions/search.js` | `openSearchViaDropDown()` |
| `FavoritesFunctions` | `business-functions/favorites.js` | Favorites management |
| `PropertyDetailsFunctions` | `business-functions/propertyDetails.js` | Property interactions |
| `ProfileFunctions` | `business-functions/profile.js` | Profile management |

### Page Objects (USE EXISTING SELECTORS)

| Page Object | File | Key Locators |
|-------------|------|--------------|
| `homePage` | `homePage.js` | `signInButton`, `userProfile`, `buyRentDropDown`, `newSearchOption` |
| `loginPage` | `loginPage.js` | `emailInput`, `passwordInput`, `signInButton` |
| `searchPanel` | `searchPanel.js` | `searchInputField`, `homeTypesButton`, `numberOfListings` |
| `propertyDetails` | `propertyDetails.js` | Property detail selectors |
| `plannerPage` | `planner.js` | Planner selectors |
| `favoritesPage` | `favoritesPage.js` | Favorites list selectors |
| `profilePage` | `profilePage.js` | Profile settings selectors |
...
// Business Functions
Pomanager.generalFunctions()         // → GeneralFunctions
Pomanager.loginFunctions()           // → LoginFunctions
Pomanager.SearchFunctions()          // → Search
Pomanager.favoritesFunctions()       // → FavoritesFunctions
Pomanager.propertyDetailsFunctions() // → PropertyDetailsFunctions
Pomanager.profileFunctions()         // → ProfileFunctions

// Page Objects
Pomanager.homePage()                 // → HomePage
Pomanager.loginPage()                // → LoginPage
Pomanager.searchPanel()              // → SearchPanel
Pomanager.propertyDetails()          // → PropertyDetails
Pomanager.plannerPage()              // → Planner
Pomanager.favoritesPage()            // → FavoritesPage
Pomanager.profilePage()              // → ProfilePage
Pomanager.skipAllComparePopUp()      // → CompareAllPopUp
Pomanager.authenticationPopUp()      // → AuthenticationPopUp
Pomanager.welcomePopUp()             // → WelcomePopUp
Pomanager.agentBranding()            // → AgentBranding
Pomanager.addNotesWindow()           // → AddNotesWindow
Pomanager.detailsCard()              // → DetailsCard
Pomanager.notificationsMenuDropDown() // → NotificationsMenuDropDown
Pomanager.logoutPage()               // → LogoutPage
```

### Test Data (USE EXISTING DATA)

```javascript
const { userTokens, baseUrl } = require('../../test-data/testData');

// Available tokens
userTokens.registered       // Registered user
userTokens.unregistered     // Unregistered user
userTokens.canopy           // Canopy user
userTokens.yesmls           // YesMLS user

// Credentials (for login scenarios)
const { firstUser } = require('../../test-data/testData').usersForBrowsers.chrome;
// firstUser.email, firstUser.password
```
...
1. IDENTIFY ACTION from test step
   ...
   ├─► "navigate"/"open" → CHECK business-functions/general.js → USE openOneHome()
   ├─► "search" → CHECK business-functions/search.js → USE openSearchViaDropDown()
   ├─► "click sign in button" → CHECK pageobjects/homePage.js → USE homePage.signInButton
   ├─► "enter email" → CHECK pageobjects/loginPage.js → USE loginPage.emailInput
   └─► No match → Generate minimal custom code + comment: // TODO: Consider adding to framework
```
...
let Pomanager;
let agentBrandingPage, skippAllComparePopUp;
let generalFunctions, loginFunctions;
let homePage, loginPage, searchPanel;
...
test.describe.serial('Test Suite Name', () => {
    ...
    test('should perform test action', async () => {
        ...
        await loginPage.emailInput.fill(firstUser.email);

        // Assertions using auto-retrying patterns
...
## 🚀 AUTO-EXECUTE TEST (After Script Generation)

```bash
npx playwright test <scriptPath> --reporter=list --headed
```

If **PASSED** on first run → Done.

If **FAILED** → Proceed to retry logic.

---

## 🔄 INTELLIGENT RETRY LOGIC (2 Iterations Max)

### Iteration 1: Quick Fix

| Error | Fix |
|-------|-----|
| `locator resolved to N elements` | Add `.first()` |
| `page closed` | Add stability checks |
| `timeout exceeded` | Increase timeout, add `waitForLoadState()` |
| `element not visible` | Check for modals, add dismissal |
| `selector not found` | Try `data-testid` or `aria-label` |

Apply fixes → re-execute.

### Iteration 2: Live Re-Exploration

- Navigate to the failure point using MCP tools
- Capture fresh selectors from the live DOM
- Replace failing selectors with fresh ones
- Re-execute
...
- [ ] `agentBranding.js` imported if welcome modal detected
- [ ] `POmanager.js` imported for all page object access
- [ ] `agentBrandingPage` initialized in `test.beforeAll`
- [ ] `skippAllComparePopUp` initialized from POmanager
- [ ] NO custom `[data-test-id="welcome-modal"]` selectors
- [ ] NO custom `.tour-step` selectors
- [ ] Framework patterns used BEFORE custom handlers
...
await generalFunctions.openOneHome(userTokens.registered);
// OR
await page.goto(`${baseUrl}&token=${userTokens.agentForRegisteredYESMLS}`);
```
