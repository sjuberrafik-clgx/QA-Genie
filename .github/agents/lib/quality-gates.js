...
const QualityGates = {
    ...
    validateMCPExploration: (workflow, ticketId) => {
        ...
        try {
            const wfConfig = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'agentic-workflow', 'config', 'workflow-config.json'), 'utf8'));
        ...
        const hasSemanticSelectors = hasDeepExploration &&
            Array.isArray(exploration.deepExploration.semanticSelectors) &&
            exploration.deepExploration.semanticSelectors.length > 0;
        const hasExtractedContent = hasDeepExploration &&
            Array.isArray(exploration.deepExploration.extractedContent) &&
            exploration.deepExploration.extractedContent.length > 0;
        const hasVerifiedUrls = hasDeepExploration &&
            Array.isArray(exploration.deepExploration.verifiedUrls) &&
            exploration.deepExploration.verifiedUrls.length > 0;
        ...
        const checks = {
            ...
            hasSemanticSelectors: hasSemanticSelectors,
            hasExtractedContent: hasExtractedContent,
            hasVerifiedUrls: hasVerifiedUrls,
        ...
        return {
            ...
            deepExploration: {
                semanticSelectors: exploration.deepExploration?.semanticSelectors?.length || 0,
                extractedContent: exploration.deepExploration?.extractedContent?.length || 0,
                verifiedUrls: exploration.deepExploration?.verifiedUrls?.length || 0
            },
