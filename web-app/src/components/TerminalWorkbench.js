...
import TerminalXtermPane from '@/components/TerminalXtermPane';
...
export default function TerminalWorkbench() {
    ...
    // Perf: debounce layout persistence so rapid toggles (resize, pane switch,
    // font-size step) don't trigger one synchronous JSON.stringify+localStorage
    // write per state change. Flushed on unmount / tab-hide.
    ...
    }), [activeSessionId, autoScroll, cols, cwd, fontSize, openSessionIds, paneMode, rows, secondarySessionId, shell, showLegacyCommandInput, themePreset]);
    useDebouncedStorage(TERMINAL_LAYOUT_STORAGE_KEY, persistedLayout, { enabled: layoutHydrated, delay: 800 });
