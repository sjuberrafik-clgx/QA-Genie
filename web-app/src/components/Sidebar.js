...
function ConnectionIndicator({ compact = false }) {
    ...
    if (compact) {
        return <span className={`status-dot ${dotClass}`} title={label} aria-label={label} />;
    }
        ...
        <aside className="fixed left-0 top-0 z-40 flex h-full w-[84px] flex-col glass-sidebar sm:w-[92px] 2xl:w-[260px]">
            ...
            <div className="border-b border-surface-200 px-3 py-4 sm:px-4 2xl:px-5 2xl:py-5">
                <Link href="/" className="block rounded-[22px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400/40">
                    <div className="rounded-[22px] border border-surface-200/80 bg-white/80 px-2.5 py-3 shadow-[0_12px_30px_rgba(15,23,42,0.06)] backdrop-blur-sm transition-transform duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(124,58,237,0.12)] sm:px-3 2xl:px-3.5">
                        <div className="flex items-center justify-center gap-3.5 2xl:justify-start">
            ...
            {/* Navigation */}
            <nav className="flex-1 space-y-0.5 px-2 py-4 sm:px-2.5 2xl:px-3">
                <p className="mb-2 hidden px-3 type-nav-section 2xl:block">Navigation</p>
                {NAV_ITEMS.map(({ to, label, Icon }) => {
                    ...
                    return (
                        <Link
                            ...
                            title={label}
                            aria-label={label}
                            className={`group relative flex items-center justify-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all duration-150 2xl:justify-start ${active
                        ...
                        >
                            ...
                            <span className="sr-only type-nav-item 2xl:not-sr-only">{label}</span>
                            {active && (
                                <>
                                    <span className="ml-auto hidden h-1.5 w-1.5 rounded-full bg-brand-500 2xl:block" />
                                    <span className="absolute right-1.5 top-1/2 block h-5 w-0.5 -translate-y-1/2 rounded-full bg-brand-500 2xl:hidden" />
                                </>
                            )}
                        </Link>
                    );
            ...
            {/* Footer */}
            <div className="border-t border-surface-200 px-3 py-4 sm:px-4">
                ...
                <div className="mt-2.5 flex items-center justify-center 2xl:justify-between">
